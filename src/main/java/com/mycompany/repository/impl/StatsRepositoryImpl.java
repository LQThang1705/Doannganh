...
public class StatsRepositoryImpl implements StatsRepository{
    ...
    @Override
    public List<Object[]> linhkienStats(String kw, Date fromDate, Date toData) {
       Session session = this.sessionFactory.getObject().getCurrentSession();
    CriteriaBuilder builder= session.getCriteriaBuilder();
    CriteriaQuery<Object[]> query= builder.createQuery(Object[].class);
    Root rootlk= query.from(Linhkien.class); 
...
//    query.groupBy(rootlk.get("maLK"));
//    query.where(predicates.toArray(new Predicate[]{}));
    query.multiselect(rootlk.get("maLK"), rootlk.get("tenLK"),builder.sum(builder.prod(rootO.get("thanhGia"), rootO.get("soLuong"))));
    query.where(builder.equal(rootO.get("maLK10"), rootlk.get("maLK")));    
    query.groupBy(rootlk.get("maLK"));

...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
  <ul class="navbar-nav">
    ...
    <sec:authorize access="hasRole('ROLE_ADMIN')">
        <li class="nav-item active">
            <a href="<c:url value="/admin/Linhkien"/>" class="nav-link">Quản lí linh kiện</a>
        </li>
    </sec:authorize>
    <sec:authorize access="hasRole('ROLE_ADMIN')">
        <li class="nav-item active">
            <a href="<c:url value="/admin/linhkien-stats"/>" class="nav-link">Thống kê</a>
        </li>
    </sec:authorize>
     <c:forEach var="c" items="${Loailk}">
        <li class="nav-item">
            <c:url value="/" var="catPath">
                <c:param name="CateID" value="${c.maLoaiLK}"></c:param>
            </c:url>
